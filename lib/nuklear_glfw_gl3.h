...
nk_glfw3_new_frame(struct nk_glfw* glfw)
{
...
#ifdef __APPLE__
    {
        ...
        int cmd_held = glfwGetKey(win, GLFW_KEY_LEFT_SUPER) == GLFW_PRESS ||
                       glfwGetKey(win, GLFW_KEY_RIGHT_SUPER) == GLFW_PRESS;
        int opt_held = glfwGetKey(win, GLFW_KEY_LEFT_ALT) == GLFW_PRESS ||
                       glfwGetKey(win, GLFW_KEY_RIGHT_ALT) == GLFW_PRESS;

        if (cmd_held) {
            /* Cmd+key: copy/paste/undo/redo/select-all + line navigation + line delete */
            ...
            if (k_state[NK_KEY_LEFT] >= 0) nk_input_key(ctx, NK_KEY_TEXT_LINE_START, k_state[NK_KEY_LEFT]);
            if (k_state[NK_KEY_RIGHT] >= 0) nk_input_key(ctx, NK_KEY_TEXT_LINE_END, k_state[NK_KEY_RIGHT]);
            /* Cmd+Backspace → delete to line start */
            if (k_state[NK_KEY_BACKSPACE] >= 0) {
                nk_input_key(ctx, NK_KEY_TEXT_LINE_BACKSPACE, k_state[NK_KEY_BACKSPACE]);
                k_state[NK_KEY_BACKSPACE] = -1; /* consume so plain backspace doesn't fire */
            }
        } else if (opt_held) {
            ...
            if (k_state[NK_KEY_BACKSPACE] >= 0) {
                nk_input_key(ctx, NK_KEY_TEXT_WORD_BACKSPACE, k_state[NK_KEY_BACKSPACE]);
                k_state[NK_KEY_BACKSPACE] = -1; /* consume */
            }
            nk_input_key(ctx, NK_KEY_COPY, 0);
            nk_input_key(ctx, NK_KEY_PASTE, 0);
            nk_input_key(ctx, NK_KEY_CUT, 0);
        } else {
            /* No modifier: plain arrow keys */
